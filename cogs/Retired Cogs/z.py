...
class Levi_Server_Stuff(commands.Cog):
    ...
    async def rat(self, ctx):
        picture_choice = random.choice(os.listdir("./Images/Cute rat pictures"))
        picture = discord.File(f"./Images/Cute rat pictures/{picture_choice}", filename="image.png")
    ...
    async def relationship(self, ctx):
        if ctx.guild.id == 819385452493537280:
            picture = discord.File(f"./Audio/Images/Relationship.png", filename="image.png")
            await ctx.send(file=picture)
    @commands.command()
    async def boiyo(self, ctx):
        if ctx.guild.id == 819385452493537280:
            picture = discord.File(f"./Audio/Images/Boiyo.png", filename="image.png")
            await ctx.send(file=picture)

async def setup(client):
    await client.add_cog(Levi_Server_Stuff(client))
