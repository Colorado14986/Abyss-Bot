...
class listeners(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.content == "Are you sure that's your code? Link codes are 4 numbers long.":
...
'''
        ...
        if message.author.id != 796467856677142530:
            ...
            '''
                ...
                #await message.channel.send('Praise him')
            if message.guild.id not in (915014434546659360, 761652116375994448):
                if '4:20' in message.content:
                    if 'https' not in message.content and '>' not in message.content:
            ...
            if message.guild.id != 915014434546659360:
                if 'ahoy ladies' in message.content or 'ahoy laddies' in message.content or 'Ahoy ladies' in message.content or 'Ahoy laddies' in message.content or 'Ahoy Ladies' in message.content or 'Ahoy Laddies' in message.content:
                    await message.channel.send('https://tenor.com/view/steve-joe-kerry-scoop-ahoy-ahoy-stranger-things-gif-17227867')
            if ('Nanomachines' in message.content) and message.guild.id != 915014434546659360:
                await message.channel.send("https://tenor.com/view/nanomachines-son-metal-gear-senator-armstrong-metal-gear-rising-revengeance-gif-20057254")
        ...
        if 'ahoy' in message.content or 'Ahoy' in message.content:
